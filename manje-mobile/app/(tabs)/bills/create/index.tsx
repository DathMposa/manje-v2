...
export default function CreateBillScreen() {
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.background.base }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScreenHeader title="Add Bill" showBack onBackPress={() => router.back()} />
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.xl, paddingBottom: spacing['4xl'] }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <ClayCard variant="subtle" style={styles.remindersCard}>
          <View style={styles.switchContainer}>
            <View>
              ...
              <Text style={[typography.body.medium, { color: colors.text.secondary }]}>Save whether this bill should stay reminder-enabled.</Text>
            </View>
        ...
        <View style={styles.actions}>
          <Button title="Save Bill" onPress={() => void handleSave()} fullWidth size="lg" variant="primary" loading={isLoading} disabled={!name.trim() || !amount || !nextDueDate.trim()} />
        </View>
      </ScrollView>
