...
export default function EditBudgetScreen() {
  ...
  const [categories, setCategories] = useState<{ id: string; name: string; limit: string; spent: number; icon: string; catKey: string; }[]>([]);

  useEffect(() => {
    if (budget) {
      setName(budget.name);
      setTotalIncome(budget.totalLimit.toString());
      setIsPrimary(budget.isPrimary);
  ...
  const handleUpdateCategory = (catId: string, limit: string) => {
    setCategories(categories.map(cat => 
      cat.id === catId ? { ...cat, limit: limit.replace(/[^0-9]/g, '') } : cat
    ));
  };
  ...
  const handleAddCategory = () => {
    ...
    setCategories([...categories, { id: newId, name: 'New Category', limit: '', spent: 0, icon: 'circle', catKey: 'other' }]);
  };
